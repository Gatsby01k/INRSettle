...
const OPERATING_MODE_LABEL: Record<SettlementMode, string> = {
  EVIDENCE_ONLY: "Evidence-only",
  PROVIDER_OBSERVED: "Provider-observed",
  CONTROLLED_PILOT: "Controlled pilot",
};

const OPERATING_MODE_DESCRIPTION: Record<SettlementMode, string> = {
  EVIDENCE_ONLY: "Evidence is reviewed without a provider execution instruction.",
  PROVIDER_OBSERVED: "Provider activity is observed and verified without automated submission.",
  CONTROLLED_PILOT: "Approved provider submission is limited by explicit pilot guardrails.",
};
...
async function setMode(formData: FormData) {
  ...
  if (!canApproveSettlement(membership.role)) {
    redirect(`/settlements/${settlementId}/controls?error=${encodeURIComponent("Only approvers can change the settlement mode.")}`);
  }
  if (!isSettlementMode(newMode)) {
    redirect(`/settlements/${settlementId}/controls?error=${encodeURIComponent("Unknown settlement mode.")}`);
  }
  ...
  // Daily cap input for CONTROLLED_PILOT entry: today's other CONTROLLED_PILOT volume.
  ...
  if (violations.length > 0) {
    redirect(
      `/settlements/${settlementId}/controls?error=${encodeURIComponent(`Cannot apply ${OPERATING_MODE_LABEL[newMode as SettlementMode]} controls: ${violations.join(" ")}`)}`,
    );
  }
  ...
  revalidatePath(`/settlements/${settlementId}/controls`);
  revalidatePath("/settlements");
  redirect(`/settlements/${settlementId}/controls?success=mode`);
}
...
async function recordManualProof(formData: FormData) {
  ...
  if (!canWriteSettlement(membership.role)) {
    redirect(`/settlements/${settlementId}/controls?error=${encodeURIComponent("Your role cannot record provider proof.")}`);
  }
  ...
  try {
    ...
    redirect(`/settlements/${settlementId}/controls?error=${encodeURIComponent(friendlyErrorMessage(error))}`);
  }

  revalidatePath(`/settlements/${settlementId}/controls`);
  redirect(`/settlements/${settlementId}/controls?success=proof`);
}
...
async function approveFinality(formData: FormData) {
  ...
  if (!canApproveSettlement(membership.role)) {
    redirect(`/settlements/${settlementId}/controls?error=${encodeURIComponent("Only approvers can approve finality.")}`);
  }
  ...
  if (mfaViolation) {
    redirect(`/settlements/${settlementId}/controls?error=${encodeURIComponent(mfaViolation)}`);
  }
  ...
  if (settlement.createdById === user.id) {
    redirect(
      `/settlements/${settlementId}/controls?error=${encodeURIComponent(
  ...
  if (existing) {
    redirect(`/settlements/${settlementId}/controls?success=finality_approved`);
  }
...
export default async function SettlementControlsPage({
  params,
  searchParams,
}: {
  ...
  const safety = {
    ...
    ...(isLiveTest
      ? {
          withinDailyCap:
            dailyUsedInrExcludingThis + inrLegOf(settlement) <= config.controlledPilotDailyMaxInr,
          dailyCapLabel: `INR ${config.controlledPilotDailyMaxInr.toLocaleString("en-IN")}`,
        }
      : {}),
  };
  const checklist = buildShadowChecklist(
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      {query.success === "mode" ? <FlashMessage message="Operating controls updated and recorded in the audit trail." /> : null}
      {query.success === "proof" ? (
      ...
      <div className="ops-panel space-y-5 p-5">
        ...
        <div className="rounded-xl border border-[var(--ops-line)] p-4">
          ...
          />
          <StatRow
            label="Automated provider submission"
            value={safety.executionBoundaryConfirmed ? "Integrated provider" : "Not confirmed"}
          />
          {canChangeMode ? (
            <form action={setMode} className="mt-3 flex flex-wrap items-end gap-2">
              <input type="hidden" name="settlementId" value={settlement.id} />
              <div className="grid gap-1.5">
                <Label htmlFor="mode">Execution posture</Label>
                <FormSelect
                  name="mode"
                  defaultValue={mode}
                  options={SETTLEMENT_MODES.map((value) => ({
                    value,
                    label: `${OPERATING_MODE_LABEL[value]}${
                      value === "PROVIDER_OBSERVED"
                        ? ""
                        : value === "CONTROLLED_PILOT"
                          ? ` (cap ${config.controlledPilotMaxInr.toLocaleString("en-IN")} INR)`
                          : ""
                    }`,
                  }))}
                />
              ...
              <p className="basis-full text-xs text-slate-400">
                Controlled-pilot entry checks limits, allowlists, beneficiary data and approval. Proof and
                independent reconciliation remain mandatory for finality.
              </p>
            </form>
          ) : (
            <p className="mt-2 text-xs text-slate-400">Only approvers can change execution controls.</p>
          )}
        </div>
          ...
          {canRecordProof ? (
            <form action={recordManualProof} className="grid gap-2.5 sm:grid-cols-2">
              ...
              <div className="grid gap-1.5">
                ...
                <Input id="provider" name="provider" placeholder={settlement.provider ?? "Provider legal name"} />
              </div>
              <div className="grid gap-1.5">
              ...
              <div className="grid gap-1.5">
                ...
                <Input id="transactionId" name="transactionId" placeholder="Provider transaction reference" />
              </div>
              <div className="grid gap-1.5">
                <Label htmlFor="utr">UTR / bank reference</Label>
                <Input id="utr" name="utr" placeholder="Bank UTR, when supplied" />
              </div>
              <div className="grid gap-1.5">
              ...
              <div className="grid gap-1.5">
                ...
                <Input id="note" name="note" placeholder="Verification source and operator note" />
              </div>
